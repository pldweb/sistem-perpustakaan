...
@section('content')
        ...
        <div class="page-inner">
          <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
            <div>
              <h3 class="fw-bold mb-3">{{ $title }} <span style="text-decoration: underline">{{ $judulBuku }}</span></h3>
...
{{--                              <th>Denda</th>--}}
                          ...
                          @foreach ($data as $index => $item)
                              <tr>
                                  ...
                                  <td><span class="badge badge-secondary">{{ $item->tanggal_pengembalian_peminjaman }}</span></td>
                                  <td>
                                      @if($item->tanggal_pengembalian_pengembalian === null)
                                          <span class="badge badge-danger">Belum Dikembalikan</span>
                                        @else
                                          <span class="badge badge-info">{{ $item->tanggal_pengembalian_pengembalian }}</span>
                                      @endif
                                  </td>
                                  <td>
